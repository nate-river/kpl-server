...
function pic2base64(pic) {
    return 'data:img/jpg;base64,' + fs.readFileSync(pic).toString('base64')
}

function equ2base64() {
    let equs = {};
    let files = fs.readdirSync('./pics/equ');
    files.forEach((v, i) => {
        equs[path.basename(v, '.jpg')] = pic2base64('./pics/equ/' + v);
    });
    fs.writeFileSync('./equs.js', 'module.exports = ' + JSON.stringify(equs));
}

function hero2base64() {
    let files = fs.readdirSync('./pics/hero');
    let css = '';
    files.forEach((v, i) => {
        css += `.hero-${path.basename(v, '.jpg')}{background:url(${pic2base64('./pics/hero/' + v)}) no-repeat center center/100% 100%}${os.EOL}`;
    });
    fs.writeFileSync('./heros.css', css);
}
...
//     console.log(data[0].hometeam.pick);
// });
